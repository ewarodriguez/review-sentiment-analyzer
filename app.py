    ...
    if uploaded_file is not None:
        ...
        if st.button("Process Dataset", type="primary"):
            ...
            is_blank_mask = (
                ...
                (raw_df[text_column].astype(str).str.strip() == "") |
                raw_df[text_column].astype(str).str.lower().isin(["na", "n/a", "null"])
            )
            ...
            if total_docs == 0:
                ...
                if total_blanks > 0:
                    st.info(f"Found {total_blanks} completely blank rows.")
                ...
                if total_blanks > 0:
                    st.warning(f"📊 Found {total_blanks} blank rows. Moving them to a separate isolated dataframe to protect model accuracy.")
        ...
        #             st.info(f"Found {total_blanks} completely blank rows.")
        #     else:
        #         # Inform the user immediately about the split distribution
        #         if total_blanks > 0:
        #             st.warning(f"📊 Found {total_blanks} blank rows. Moving them to a separate isolated dataframe to protect model accuracy.")
